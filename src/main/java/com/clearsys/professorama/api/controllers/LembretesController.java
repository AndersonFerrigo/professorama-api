...
public class LembretesController {
	...
	@RequestMapping(value = "/buscarLembretePeloId/{id}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/buscarLembretePelaSerie/{serie}", method = RequestMethod.GET)
	...
	public ResponseEntity<Lembretes> atualizar(@PathVariable("id") long id, @Valid @RequestBody Lembretes lembretes,
	...
	private Lembretes converterDtoParaLembrete(LembretesDto lembretesDto, BindingResult result)
		...
		/*
		 * if(!atividadeDto.getId().equals(null)) { Optional<Atividade> ativ =
		 * this.atividadeService.buscarPorId(atividadeDto.getId()); if(ativ.isPresent())
		 * { atividade = ativ.get(); }else { result.addError(new
		 * ObjectError("atividade", "Atividade não encontrada."));
		 * 
		 * } }else { atividade.setProfessor(new Professor());
